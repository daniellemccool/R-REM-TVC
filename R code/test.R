...
X0     <- cbind(1,x0)                 # covariate vector at time = 0
...
N <- length(X)
Time <- ncol(Xtv)
ones <- matrix(1, ncol = Time, nrow = N)
X1 <- matrix(X, ncol = Time, nrow = N)
...
rem.mtvc.lik <- function(b, ev.mat, x.arr) {
    ...
    logl  <- sum((ev.mat == 1) * log(u.mat)) -
             sum((ev.mat != 3) * u.mat) -
#             sum(log(1 - exp(-1 * rowSums(u.mat)))) # Full unobs.
             sum(log(1 - exp(-1 * ncol(u.mat)*u.mat[,1]))) # Only t0 unobs.

    return(-logl)
}

x.arr <- array(c(ones, X1, Xtv), c(N, Time, 3), dimnames = list(NULL, paste0('d', 1:Time), paste0('X', 1:3)))
