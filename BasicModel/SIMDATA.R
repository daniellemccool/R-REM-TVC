...
simTop <- function(N,Time,b,delta,NormVar=0) {
    ...
    X0   <- cbind(1,matrix(rnorm(2*N,0,1),ncol=2)) # Nx3 covariate matrix at t=0

    XT   <- matrix(X0[,3],N,Time)              # NxT matrix with time-varying covariate x[3] at t=0
      ...
      for(j in 1:Time){
            ...
            if(runif(1) < rate[i]){
              ...
              if(NormVar==1){
                ...
                Xt[i,3]   <- rnorm(1)          # random change in TV covariate vector day after event
                
                }else{
                  
                Xt[i,3]   <- Xt[i,3]+delta     # systematic change in TV covariate vector day after event
              ...
              if(j<Time){
                XT[i,(j+1):Time] <- Xt[i,3]   # New Xtv-values day after event
              }
